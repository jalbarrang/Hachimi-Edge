...
use super::utils;
use crate::{
    android::gui_impl::input_hook,
    core::{Error, Hachimi, Interceptor},
};
...
extern "C" fn dlopen(filename: *const c_char, flags: c_int) -> *mut c_void {
    ...
    let orig_fn: DlopenFn = unsafe { std::mem::transmute(hachimi.interceptor.get_trampoline_addr(dlopen as usize)) };
...
type DoDlopenFn = extern "C" fn(
    filename: *const c_char,
    flags: c_int,
    extinfo: *const c_void,
    caller_addr: *const c_void,
) -> *mut c_void;
extern "C" fn do_dlopen(
    filename: *const c_char,
    flags: c_int,
    extinfo: *const c_void,
    caller_addr: *const c_void,
    ...
    let hachimi = Hachimi::instance();
    let orig_fn: DoDlopenFn =
        // SAFETY: Transmute required for IL2CPP type conversion
        unsafe { std::mem::transmute(hachimi.interceptor.get_trampoline_addr(do_dlopen as usize)) };
...
type RegisterNativesFn =
    extern "C" fn(env: JNIEnv, class: jclass, methods: *const JNINativeMethod, count: jint) -> jint;
#[allow(non_snake_case)]
extern "C" fn JNINativeInterface_RegisterNatives(
    env: JNIEnv,
    class: jclass,
    ...
    let orig_fn: RegisterNativesFn = unsafe {
        std::mem::transmute(
            hachimi
                .interceptor
                .get_trampoline_addr(JNINativeInterface_RegisterNatives as usize),
        )
    };
...
fn init_internal(env: *mut jni::sys::JNIEnv) -> Result<(), Error> {
    ...
    let force_hook_dlopen = hachimi.config.load().android.hook_libc_dlopen
        || std::fs::metadata("/vendor/waydroid.prop")
            .ok()
    ...
    if !hachimi.config.load().disable_gui {
        ...
        hachimi
            .interceptor
            .hook(register_natives_addr, JNINativeInterface_RegisterNatives as usize)?;
    }
