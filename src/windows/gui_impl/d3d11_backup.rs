...
use windows::Win32::{
    ...
    Graphics::{
        ...
        Direct3D11::{
            ID3D11BlendState, ID3D11Buffer, ID3D11ClassInstance, ID3D11DepthStencilState, ID3D11DeviceContext,
            ID3D11GeometryShader, ID3D11InputLayout, ID3D11PixelShader, ID3D11RasterizerState, ID3D11SamplerState,
    ...
    pub unsafe fn restore(&mut self, ctx: &ID3D11DeviceContext) {
        ...
        unsafe {
            ...
            if let Some(index_buffer) = self.index_buffer.take() {
                ctx.IASetIndexBuffer(&index_buffer, self.index_buffer_format, self.index_buffer_offset);
            }
