...
impl D3D11Painter {
    pub fn new(swap_chain: IDXGISwapChain) -> Result<D3D11Painter, Error> {
        ...
        let device: ID3D11Device = unsafe { swap_chain.GetDevice() }
            .map_err(|_| Error::RuntimeError("Failed to get D3D11 device".to_owned()))?;
        let renderer = egui_directx11::Renderer::new(&device).map_err(|e| Error::RuntimeError(e.to_string()))?;
    ...
    fn get_device(&self) -> Result<ID3D11Device, Error> {
        // SAFETY: FFI / raw pointer operation required by IL2CPP interop
        unsafe { self.swap_chain.GetDevice() }.map_err(|_| Error::RuntimeError("Failed to get D3D11 device".to_owned()))
    }

    fn get_device_context(device: &ID3D11Device) -> Result<ID3D11DeviceContext, Error> {
        // SAFETY: FFI / raw pointer operation required by IL2CPP interop
        unsafe { device.GetImmediateContext() }
            .map_err(|_| Error::RuntimeError("Failed to get D3D11 device context".to_owned()))
    }
    ...
    fn init_render_target(&mut self) {
        ...
        let render_target_desc = D3D11_RENDER_TARGET_VIEW_DESC {
        ...
        // SAFETY: FFI / raw pointer operation required by IL2CPP interop
        if let Err(e) = unsafe {
            device.CreateRenderTargetView(&backbuffer, Some(&render_target_desc), Some(&mut self.render_target))
        } {
    ...
    pub fn present(
        ...
        egui_output: egui_directx11::RendererOutput,
    ) -> Result<(), Error> {
        ...
        // SAFETY: FFI / raw pointer operation required by IL2CPP interop
        unsafe {
            self.backup_state.save(&device_context);
        }

        if let Err(e) = self
            .renderer
            .render(&device_context, render_target, egui_ctx, egui_output)
        {
        ...
        // SAFETY: FFI / raw pointer operation required by IL2CPP interop
        unsafe {
            self.backup_state.restore(&device_context);
        }
        Ok(())
    }
}
