...
use windows::{
    ...
    Win32::{
        ...
        UI::{
            Shell::{
                FileOpenDialog, IFileOpenDialog, IShellItem, SHCreateItemFromParsingName, FOS_FILEMUSTEXIST,
...
pub fn open_select_folder_dialog<P: AsRef<Path>>(owner: HWND, default_folder: Option<P>) -> Option<PathBuf> {
    let dialog: IFileOpenDialog = unsafe { CoCreateInstance(&FileOpenDialog, None, CLSCTX_INPROC_SERVER).ok()? };
